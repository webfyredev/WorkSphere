    ...
    return(
        <>
            <div className="w-full md:h-100 h-auto my-10 flex flex-col">
                ...
                <div className="w-full md:h-100 h-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 px-10 ml-5 md:ml-0 md:p-3 mt-5 md:mt-0">
                    {reasons.map((data, index) =>{
                        // const 
                        const Icon = icons[data.icon];
                        return(
                            <motion.div {...scrollLeft} {...CardHover} key={index} className="p-5 flex flex-col items-center rounded-lg w-[90%] md:w-[95%] shadow-[0_0_5px_rgba(0,0,0,0.1)] md:h-[85%] mb-5">
                                ...
                                <p className="text-xs mt-5 text-center mb-5">{data.text}</p>
                            </motion.div>
                        )
