    ...
    return(
        <>
            ...
            <motion.h2 {...ScrollUp} className="text-2xl md:text-3xl font-semibold mt-20 w-full h-10 flex justify-center items-center text-blue-500 mb-5">
             ...
             <div className="w-full h-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 p-3 mb-10 md:mb-5">
                {jobs.map((data, index) =>(
                    <motion.div {...CardHover} {...scrollLeft} className="w-[98%] h-auto p-5 flex flex-col shadow-[0_0_5px_rgba(0,0,0,0.1)] rounded-md">
                        ...
                        <div className="w-[70%] md:w-full h-auto mb-3 overflow-hidden">
                            <ul className="flex flex-row justify-between items-center">
                                {data.skills.map((skills, index) =>(
                                    <motion.li {...CardHover} key={index} className="text-[9px] md:text-[12px] px-3 sm:px-2 sm:text-[10px] md:px-3 h-7 flex items-center rounded-xs bg-gray-100 text-gray-700 shadow-[0_0_2px_rgba(0,0,0,0.1)] font-semibold hover:bg-linear-to-r from-blue-500 to-blue-600 cursor-pointer hover:text-white transition-all duration-300">
                                        {skills}
                                    </motion.li>
                                ))}
                            </ul>
